...
const manifest = JSON.parse(readFileSync(resolve(root, "pocket.3ds.json"), "utf8"));
...
await build3ds([`--plan=${planPath}`, `--manifest=${resolve(root, "pocket.3ds.json")}`, `--project-root=${root}`, ...process.argv.slice(2)]);
